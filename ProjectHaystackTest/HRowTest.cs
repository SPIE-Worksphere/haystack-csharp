...
namespace ProjectHaystackTest
{
    ...
    public class HRowTest : HValTest
    {
        ...
        public void testBasics()
        {
            ...
            Assert.IsTrue(row.missing("foo"));
            Assert.IsNull(row.get("foo", false));
            try { row.get("foo"); Assert.Fail(); } catch (UnknownNameException) { Assert.IsTrue(true); }
            try { row.get("foo", true); Assert.Fail(); } catch (UnknownNameException) { Assert.IsTrue(true); }
        }
